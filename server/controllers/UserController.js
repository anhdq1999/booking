...
class UserController {
    ...
    create(req, res, next) {
        ...
        User.findOne({ username: req.body.username })
            .then((user) => {
                if (user)
                    res.status(409).json(RES.USER.CREATE.FAILURE_EXIST());
                else {
                    ...
                    newUser = newUser.save().then(u => {
                        res.status(200).json(RES.USER.CREATE.SUCCESS(u));

                    }).catch(err=>res.status(400).json(RES.USER.CREATE.FAILURE()));
...
garbage(req, res, next) {
    User.findDeleted({})
        .then((users) => res.status(200).json(RES.USER.GETALL_DELETED.SUCCESS(users)))
        .catch(next);
}
